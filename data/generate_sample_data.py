...
def get_monthly_allocation(date):
    """Get monthly allocation from annual data with smooth year-to-year transitions"""
    year = date.year
    month = date.month

    current_emissions = ANNUAL_EMISSIONS.get(year, ANNUAL_EMISSIONS[2024])
    current_energy = ANNUAL_ENERGY.get(year, ANNUAL_ENERGY[2024])
    current_water = ANNUAL_WATER.get(year, ANNUAL_WATER[2024])
    current_waste = ANNUAL_WASTE.get(year, ANNUAL_WASTE[2024])

    next_year = year + 1
    next_emissions = ANNUAL_EMISSIONS.get(next_year, current_emissions)
    next_energy = ANNUAL_ENERGY.get(next_year, current_energy)
    next_water = ANNUAL_WATER.get(next_year, current_water)
    next_waste = ANNUAL_WASTE.get(next_year, current_waste)

    t = (month - 1) / 11  # 0 at month 1, 1 at month 12

    def interpolate(current, next_val):
        return current + (next_val - current) * t

    scope1_base = interpolate(current_emissions['scope1'], next_emissions['scope1']) / 12
    scope2_location_base = interpolate(current_emissions['scope2_location'], next_emissions['scope2_location']) / 12
    scope2_market_base = interpolate(current_emissions['scope2_market'], next_emissions['scope2_market']) / 12
    scope3_base = interpolate(current_emissions['scope3'], next_emissions['scope3']) / 12
    electricity_base = interpolate(current_energy['electricity'], next_energy['electricity']) / 12
    fuel_base = interpolate(current_energy['fuel'], next_energy['fuel']) / 12
    water_base = interpolate(current_water['consumption'], next_water['consumption']) / 12
    waste_base = interpolate(current_waste['total'], next_waste['total']) / 12

    if month in [6, 7, 8]:
        seasonal_energy = 1.03
        seasonal_water = 1.04
    elif month in [12, 1, 2]:
        seasonal_energy = 1.02
        seasonal_water = 0.98
    else:
        seasonal_energy = 1.0
        seasonal_water = 1.0
    ...
    return {
        'scope1': scope1_base,
        'scope2_location': scope2_location_base * seasonal_energy,
        'scope2_market': scope2_market_base * seasonal_energy,
        'scope3': scope3_base,
        'electricity': electricity_base * seasonal_energy,
        'fuel': fuel_base,
        'water_consumption': water_base * seasonal_water,
        'water_replenishment_pct': current_water['replenishment_pct'],
        'waste_total': waste_base,
        'waste_diversion_pct': current_waste['diversion_pct'],
        'renewable_pct': 1.00,
    }

def generate_sustainability_database(db_path):
    """Generate database with actual operational metrics (writes ONLY to db_path)."""
    ...
    # Delete existing database file (to avoid stale/empty DBs)
    if DB_PATH.exists():
    ...
    cursor.executemany(
        'INSERT INTO emission_factors VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?)',
        factors
    )
    ...
    cursor.executemany(
        'INSERT INTO emission_targets VALUES (NULL, ?, ?, ?, ?, ?, ?, ?)',
        targets
    )
    ...
    while current_date <= end_date:
        ...
        for facility in facilities:
            ...
            scope3_val = monthly['scope3'] * facility_fraction * variation
            ...
            if facility_type == 'Data Center' and facility['id'] in FACILITY_PUE_2024:
                ...
                pue = pue_2024 + (years_from_2024 * 0.002)
            else:
                pue = None
            ...
            energy_cost = electricity * ENERGY_COSTS.get(region, 70) / 1000
            water_cost = water_consumption * WATER_COSTS.get(region, 8.5) / 1000
            carbon_cost = (scope1 + scope2_market) * CARBON_PRICING.get(region, 0) / 1000
...
if __name__ == "__main__":
    # Local usage only (Streamlit should call generate_sustainability_database(db_path=...))
    BASE_DIR = Path(__file__).resolve().parent.parent
    DB_PATH = BASE_DIR / "data" / "sustainability_data.db"
    generate_sustainability_database(DB_PATH)
